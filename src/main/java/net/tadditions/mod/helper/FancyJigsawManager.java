...
public class FancyJigsawManager {
    ...
    public static void assembleJigsawStructure(
        ...
        // Get starting pool
        JigsawPattern startPool = FancyJigsawConfig.getStartPoolSupplier().get();
        ...
        if (FancyJigsawConfig.getMaxChainPieceLength() > 0) { // Realistically this should always be true. Why make a jigsaw config with a non-positive size?
            AxisAlignedBB axisAlignedBB = new AxisAlignedBB(pieceCenterX - 4700, pieceCenterY - 4700, pieceCenterZ - 4700, pieceCenterX + 4700 + 1, pieceCenterY + 4700 + 1, pieceCenterZ + 4700 + 1);
            Assembler assembler = new Assembler(jigsawPoolRegistry, FancyJigsawConfig.getMaxChainPieceLength(), chunkGenerator, templateManager, components, random);
        ...
        public void processPiece(AbstractVillagePiece piece, MutableObject<VoxelShape> voxelShape, int boundsTop, int depth, boolean doBoundaryAdjustments) {
            ...
            for (Template.BlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (!(poolOptional.isPresent() && (poolOptional.get().getNumberOfPieces() != 0 || Objects.equals(jigsawBlockPool, JigsawPatternRegistry.field_244091_a.getLocation())))) {
                    QolMod.LOGGER.warn("Empty or nonexistent pool: {}", jigsawBlockPool);
                    continue;
                }
                ...
                if (!(fallbackOptional.isPresent() && (fallbackOptional.get().getNumberOfPieces() != 0 || Objects.equals(jigsawBlockFallback, JigsawPatternRegistry.field_244091_a.getLocation())))) {
                    QolMod.LOGGER.warn("Empty or nonexistent fallback pool: {}", jigsawBlockFallback);
                    continue;
                }
