...
public class SolenoidFilledModel extends AnimatedGeoModel<SolenoidFilledTile> {
    ...
    @Override
    public ResourceLocation getModelLocation(SolenoidFilledTile object) {
        return modelResource;
    }

    @Override
    public ResourceLocation getTextureLocation(SolenoidFilledTile object) {
        return animateTexture(animo);
    }

    @Override
    public ResourceLocation getAnimationFileLocation(SolenoidFilledTile object) {
        return animationResource;
    }

    public ResourceLocation animateTexture(List<ResourceLocation> list){
        tickCount++;
        tickCount += ((int) Minecraft.getInstance().getRenderPartialTicks());
        if (tickCount >= 70) {
            tickCount = 0;
            currentIndex++;
